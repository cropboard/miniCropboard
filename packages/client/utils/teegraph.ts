class TeeGraph {
  ...
  reduceData(data: Array<number>): Array<Array<number>> {
    ...
    const sumOfData: number =
      data.length !== 0 ? data.reduce((total, current) => total + current) : 0;

    const proportionalData: Array<number> = data.map((unitData) => {
      return Math.floor((unitData / sumOfData) * this.width);
    });

    const sumOfProportion: number =
      data.length !== 0 ? data.reduce((total, current) => total + current) : 0;
  ...
  prepareRects(): string {
    ...
    for (let d: number = 0; d < this.data.length; d++) {
      ...
      let offsetsSum: number = this.offsets.reduce(
        (total, current) => total + current
      );
      rects__.push(
        `<rect x="${this.offsets[this.offsets.length - 1] + 1}" y="0" width="${
          (dataPercentile[d] / 100) * this.width
        }px" height="10%" fill="${color}" />\n`
      );
      this.offsets.push(
        (dataPercentile[d] / 100) * this.width +
      ...
      console.log(
        `Offsets -> ${JSON.stringify(this.offsets)} Per -> ${
          (dataPercentile[d] / 100) * this.width
        }`
      );
    }
  ...
  prepareLabels(): string {
    ...
    for (let d = 0; d < this.data.length; d++) {
      ...
      chartLabels__.push(
        `<text font-size="50%" font-family="sans-serif" x="${
          this.offsets[d] + d * 8
        }" y="25" fill="rgb(0,0,0)"> ${this.data[d]} </text>\n`
      );
