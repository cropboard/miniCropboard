...
const resolvers = {
  ...
  Mutation: {
    ...
    updateFarm: async (parent: any, args: any) => {
      ...
      if (!hasFailed) {
        ...
        unval_ = await Farm.findByIdAndUpdate(
          farmId,
          {
    ...
    createCropData: async (parent: any, args: any) => {
      ...
      unval = await Crop.findById(
        parentCropId,
        (error: any, parentcrop: any) => {
          if (error) {
            isError = true;
          }
    ...
    harvestCrop: (parent: any, args: any) => {
      ...
      Crop.findByIdAndUpdate(
        cropId,
        { harvested: true, output: cropOutput },
        (error: any, res: any) => {
          if (error) {
            isError = true;
            return "Error";
  ...
  User: {
    farms: async (parent: any, args: any) => {
      ...
      let farms;
      let unval; // waiter
      let farmFetch = await Farm.find(
        { owner: parent._id },
        (error: any, farms_: any) => {
          // handling fail safe
          if (error) {
            isError = true;
            return Error(error);
  ...
  Farm: {
    crops: async (parent: any, args: any) => {
      ...
      let unval;
      // console.log(parent)
      let cropsFetch = await Crop.find(
        { farm: parent._id },
        (error: any, crops_: any) => {
          if (error) {
            isError = true;
            return Error(error);
          }
  ...
  Crop: {
    cropsData: async (parent: any, args: any) => {
      ...
      let cropsDataFetch = await CropData.find(
        { crop: parent._id },
        (error: any, cropsdata: any) => {
          if (error) {
            isError = true;
            return Error(error);
          }
